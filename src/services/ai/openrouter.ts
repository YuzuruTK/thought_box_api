/** OpenRouter chat completions client with retry, timeout and safe error handling. */
...
export class AiProviderError extends Error {
  ...
  constructor(message: string, readonly status?: number, readonly retryAfterSeconds?: number, readonly providerCode?: number, readonly providerName?: string) {
    super(message);
...
export interface CompletionOptions { apiKey: string; model: string; messages: ChatMessage[]; maxTokens: number; temperature?: number; }

interface OpenRouterErrorBody { error?: { code?: number; message?: string; metadata?: Record<string, unknown> }; }
interface OpenRouterSuccessBody { choices?: Array<{ message?: { content?: string | null } }>; }
...
function truncate(v: string, max: number): string { return v.length <= max ? v : `${v.slice(0, max)}…`; }
...
async function attemptRequest(options: CompletionOptions): Promise<string> {
  const response = await fetch(OPENROUTER_URL, {
    ...
    body: JSON.stringify({ model: options.model, messages: options.messages, max_tokens: options.maxTokens, temperature: options.temperature ?? 0.3 }),
  ...
  if (!response.ok) {
    ...
    const providerName = typeof errorMetadata?.provider_name === "string" ? errorMetadata.provider_name : undefined;
    console.warn(`[openrouter] ${response.status} ${response.statusText} | model=${options.model}`, {
      status: response.status, statusText: response.statusText, model: options.model, errorCode,
      errorType: errorMetadata?.error_type, providerName, retryAfter, body: truncate(bodyText, 500),
    });
  ...
  if (typeof content !== "string" || content.trim().length === 0) throw new AiProviderError("OpenRouter returned an empty completion.", 502);
  return content;
}
...
export async function chatCompletion(options: CompletionOptions): Promise<string> {
  ...
  for (let attempt = 0; attempt <= MAX_RETRIES; attempt++) {
    try {
      ...
      if (error instanceof DOMException && (error.name === "TimeoutError" || error.name === "AbortError")) lastError = new AiTimeoutError();
      else lastError = error instanceof Error ? error : new AiProviderError(String(error));
      const retryable =
      ...
      const waitSecs = lastError instanceof AiProviderError && lastError.retryAfterSeconds !== undefined ? lastError.retryAfterSeconds : undefined;
      if (waitSecs !== undefined) {
        if (waitSecs <= MAX_RETRY_AFTER_SECONDS) await sleep(waitSecs * 1_000);
        else break;
      } else await sleep(2 ** attempt * 500);
    }
  }
  throw lastError;
}
